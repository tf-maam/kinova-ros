...
arm_joint_number = 0
finger_number = 0
prrefix = 'NO_ROBOT_TYPE_DEFINED_'
...
def joint_angle_publisher(angle_set):
    """Send a joint angle goal to the action server."""
    action_address = '/' + prefix + 'driver/joints_action/joint_angles'
    client = actionlib.SimpleActionClient(action_address,
                                          kinova_msgs.msg.ArmJointAnglesAction)
...
def setcurrentJointCommand(feedback):
    global currentJointCommand, joint_command_history
...
def argumentParser(argument):
    ...
    parser.add_argument('kinova_robotType', metavar='kinova_robotType', type=str, default='j2s7s300',
                        help='kinova_RobotType is in format of: [{j|m|r|c}{1|2}{s|n}{4|6|7}{s|a}{2|3}{0}{0}]. eg: j2s7s300 refers to jaco v2 6DOF assistive 3fingers. Please be noted that not all options are valided for different robot types.')
    ...
    parser.add_argument('-b', '--back_home', action='store_true',
                            help='Move the robot to the home position ([102.1, 197.3, 180.8, 43.6, 265.3, 257.1])')
...
def apply_joint_degrees(joint_degree):
    positions = [0]*7
    try:
...
if __name__ == '__main__':
    ...
    if args.back_home:
        ...
        """
        home = np.array([102.1, 197.3, 180.8, 43.6, 265.3, 257.1])
        dp = np.array([1, 1, 1, 1, 1, 1])
        repititions = 20
        simple_trajectory = [home + i * dp for i in range(repititions)]
        
        for position in simple_trajectory:
        ...
        joint_target_degree, joint_radian = unitParser(args.unit, args.joint_value, args.relative)
